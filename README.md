...
- PostgreSQL-хранилище для `max_events`, `dialog_sessions`, `linked_accounts`, `meter_readings` и `appeals`; без `DATABASE_URL` доступен только временный in-memory режим для разработки.
...
В логах бота должна быть строка `using PostgreSQL store`. Если вместо нее видно `using in-memory store`, значит `DATABASE_URL` не передан в процесс бота и данные будут храниться только до перезапуска:

```bash
docker-compose logs max-bot | tail -50
```
### Как подключается база данных

Бот подключает PostgreSQL только через переменную окружения `DATABASE_URL`.

```dotenv
DATABASE_URL=postgres://maxbot:maxbot_password@localhost:5433/maxbot?sslmode=disable
```

Правила простые:

- `DATABASE_URL` задан — при старте в логах будет `using PostgreSQL store`, бот подключится к PostgreSQL и будет сохранять события, сессии, привязанные ЛС, показания и обращения;
- `DATABASE_URL` пустой или отсутствует — при старте в логах будет `using in-memory store (for development only)`, база данных не используется, все состояние пропадет после перезапуска;
- схема таблиц применяется из `init-db.sql`: в Docker она применяется автоматически при первом создании volume, а для локальной PostgreSQL ее применяют скрипты из раздела 2b.

Для Docker из этого репозитория DSN внутри контейнера уже прописан в `docker-compose.yml`:

```text
postgres://maxbot:maxbot_password@postgres:5432/maxbot?sslmode=disable
```

Для подключения с хоста к Docker-PostgreSQL используйте порт `5433`:

```bash
psql "postgres://maxbot:maxbot_password@localhost:5433/maxbot?sslmode=disable"
```
...
После запуска напишите боту в MAX `/start`. До авторизации бот должен предложить кнопку «Авторизоваться» и не должен показывать персональные разделы. Затем нажмите «Авторизоваться» или отправьте `авторизоваться`, введите `000123456`, `1234`; только после этого должны стать доступны `баланс`, `показания`, `внести показания`, `обращение`, `справка`. Ответы будут отправляться в настоящий чат MAX, а данные 1С будут браться из локального mock.
...
curl -s -X POST http://localhost:8080/debug/send-test-update \
  -H 'Content-Type: application/json' \
  -d '{"user_id":123456789,"chat_id":987654321,"mid":"smoke-007","text":"внести показания"}'

curl -s -X POST http://localhost:8080/debug/send-test-update \
  -H 'Content-Type: application/json' \
  -d '{"user_id":123456789,"chat_id":987654321,"mid":"smoke-008","text":"показание MTR-001 245.678"}'

curl -s -X POST http://localhost:8080/debug/send-test-update \
  -H 'Content-Type: application/json' \
  -d '{"user_id":123456789,"chat_id":987654321,"mid":"smoke-009","text":"обращение не убран подъезд"}'

curl -s -X POST http://localhost:8080/debug/send-test-update \
  -H 'Content-Type: application/json' \
  -d '{"user_id":123456789,"chat_id":987654321,"mid":"smoke-010","text":"справка"}'
```
...
Проверьте, что привязанный ЛС сохранен локально:

```bash
docker-compose exec -T postgres psql -U maxbot -d maxbot -c "SELECT max_user_id, account_id, account_number, source, updated_at FROM linked_accounts ORDER BY updated_at DESC LIMIT 10;"
```

После команды передачи показания проверьте таблицу показаний:

```bash
docker-compose exec -T postgres psql -U maxbot -d maxbot -c "SELECT max_user_id, account_id, account_number, meter_id, period, value, operation_id, message_id, created_at FROM meter_readings ORDER BY created_at DESC LIMIT 10;"
```

После создания обращения проверьте таблицу обращений:

```bash
docker-compose exec -T postgres psql -U maxbot -d maxbot -c "SELECT max_user_id, account_id, account_number, appeal_id, appeal_number, text, operation_id, message_id, created_at FROM appeals ORDER BY created_at DESC LIMIT 10;"
```
