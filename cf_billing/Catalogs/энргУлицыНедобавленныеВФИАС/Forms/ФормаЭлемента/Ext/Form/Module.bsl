...
&НаКлиенте
Процедура НоваяУлицаПриИзменении(Элемент)
	НоваяУлицаПриИзмененииНаСервере();
КонецПроцедуры
...
&НаСервере
Процедура НоваяУлицаПриИзмененииНаСервере()
	СтрокиКИ 							= Объект.КонтактнаяИнформация.НайтиСтроки(Новый Структура("Вид",Справочники.ВидыКонтактнойИнформации.АдресНедобавленногоОбъекта));
	Если СтрокиКИ.количество() > 0 тогда
		Объект.наименование  	= Лев(СтрокиКИ[0].Представление + Объект.НоваяУлица,150); 
	КонецЕсли;
КонецПроцедуры
