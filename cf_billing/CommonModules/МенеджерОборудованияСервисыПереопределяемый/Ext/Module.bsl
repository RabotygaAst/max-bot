...
// Процедура заполнения структуры прайс-листа 
// при выгрузке методом GetPriceList web-сервиса EquipmentService
// так же используется при выгрузке на ККМ Offline 
// при использовании драйвера 1С:ККМ Offline
// Для заполнения используются подготовленные пустые 
// структуры получаемые функциями из 
// модуля МенеджерОборудованияСервисыКлиентСервер
//
// Пример заполнения:
//// ГРУППЫ ТОВАРОВ
