...
Перем ВидОперацийНачисления Экспорт; // Заполняет таблицу периодов в случае, если вид операций начисления является непределенным
...
перем НаборОбъемНачислений Экспорт ; // Набор записей объема начислений
перем НаборОбъемНачисленийПомещений Экспорт ; // Набор записей объема начислений помещений
перем МенеджерДокументовНачислений Экспорт; // Список документов, выполняющих функцию начисления
...
Процедура Инициализировать(МВТ, ПолучаемыеНаборы) Экспорт
	...
	КонецЕсли;
	Если НЕ ПараметрыЗапроса.Найти("ТипНачисления") = Неопределено Тогда
...
Процедура ПередПроведениемМКД(МКД, яДокументСсылка, ПометкаУдаления, ТекИндексСтрокиНабора = Неопределено, ТекИндексСтрокиНабораПомещений = Неопределено) Экспорт
	Если ВидОперацийНачисления = Перечисления.энргВидыОперацийНачисления.ПерерасчетИзменениеПризнакаНаличияПрибораУчетаСНачалаГода Тогда
		...
		ИначеЕсли ПометкаУдаления Тогда
			Документы.энргПерерасчет.СнятьПометкуУдаленияСвязанныйПерерасчет(яДокументСсылка,Организация,Район,МКД); 
			РегистрыСведений.энргДокументыНачислений.СнятьПометкуУдаленияДокументаНачислений(ПериодНачисления, Организация, Район, Строение, ВидОперацийНачисления);
...
Функция ЗаполнитьПредоставленныеПоказания()
	...
	|	МаксимальныеПоказания.Период КАК ДатаРегистратора,
	|	МаксимальныеПоказания.Показание КАК Показание,
	|	МаксимальныеПоказания.ПериодНачисления КАК ПериодНачисления
	...
	|	ТаблицаПериодов.Район КАК Район,
	|	МИНИМУМ(ТаблицаПериодов.НачалоОП) КАК НачалоОП
	|ПОМЕСТИТЬ ВТ_МинНачалоОП
	|ИЗ
	|	ТаблицаПериодов КАК ТаблицаПериодов
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаПериодов.Организация,
	|	ТаблицаПериодов.Район";
	Пакет 									= Схема.ПакетЗапросов.Добавить();
	Пакет.УстановитьТекстЗапроса(Текст);
	...
	|	ВТ_МинНачалоОП.НачалоОП КАК МиниНачалоОП
	|ПОМЕСТИТЬ ВТ_ТаблицаПериодовСМинНачаломОП
	|ИЗ
	|	ТаблицаПериодов КАК ТаблицаПериодов
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_МинНачалоОП КАК ВТ_МинНачалоОП
	|		ПО ТаблицаПериодов.Организация = ВТ_МинНачалоОП.Организация
	|			И ТаблицаПериодов.Район = ВТ_МинНачалоОП.Район";
	Пакет 									= Схема.ПакетЗапросов.Добавить();
	Пакет.УстановитьТекстЗапроса(Текст);
	...
	|	МаксимальныеПоказанияНачало.Период КАК Период,
	|	МаксимальныеПоказанияНачало.Регистратор КАК Регистратор,
	|	ТаблицаПериодов.ПериодНачисления КАК ПериодНачисленияФактический,
	|	МаксимальныеПоказанияНачало.ПериодНачисления КАК ПериодНачисленияКорректировки,
	|	МаксимальныеПоказанияНачало.Район КАК Район,
	|	МаксимальныеПоказанияНачало.КлючПомещения КАК КлючПомещения,
	...
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(МаксимальныеПоказанияНачало.Регистратор) = ТИП(Документ.энргОперацияСЛицевымСчетом)
	|				И ВЫРАЗИТЬ(МаксимальныеПоказанияНачало.Регистратор КАК Документ.энргОперацияСЛицевымСчетом).ВидОперации <> ЗНАЧЕНИЕ(Перечисление.энргВидыОпераций.энргОперацияСЛС_СменаСобственника)
	|			ТОГДА ЛОЖЬ
	|		КОГДА ТИПЗНАЧЕНИЯ(МаксимальныеПоказанияНачало.Регистратор) = ТИП(Документ.энргОперацияСПриборомУчета)
	|				И &НачПокНеЯвляютсяКонПриУстПрибора
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК ВСрок,
	|	МаксимальныеПоказанияНачало.Показание КАК Показание,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(МаксимальныеПоказанияНачало.Регистратор) = ТИП(Документ.энргПерерасчет)
	|				И ВЫРАЗИТЬ(МаксимальныеПоказанияНачало.Регистратор КАК Документ.энргПерерасчет).ВидОперации = ЗНАЧЕНИЕ(Перечисление.энргВидыОпераций.энргПерерасчет_ИзменениеПараметраРассчета)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК ПоказанияТекущегоПериода
	|ПОМЕСТИТЬ МаксимальныеПоказанияНачало
	|ИЗ
	|	НаборМаксимальныеПоказания КАК МаксимальныеПоказанияНачало
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ТаблицаПериодовСМинНачаломОП КАК ТаблицаПериодов
	...
	|				ИЛИ МаксимальныеПоказанияНачало.Период < ТаблицаПериодов.МиниНачалоОП
	|					И ВЫБОР
	|						КОГДА ТИПЗНАЧЕНИЯ(МаксимальныеПоказанияНачало.Регистратор) = ТИП(Документ.энргВводНачальныхОстатков)
	|							ТОГДА ВЫБОР
	|									КОГДА МаксимальныеПоказанияНачало.ПериодНачисления = ТаблицаПериодов.ПериодНачисления
	|										ТОГДА ИСТИНА
	|									ИНАЧЕ ЛОЖЬ
	|								КОНЕЦ
	|						ИНАЧЕ ИСТИНА
	...
	"ВЫБРАТЬ
	|	МаксимальныеПоказанияНачало.Организация КАК Организация,
	|	МаксимальныеПоказанияНачало.ПериодНачисленияФактический КАК ПериодНачисления,
	|	МаксимальныеПоказанияНачало.ПериодНачисленияКорректировки КАК ПериодНачисленияКорректировки,
	|	МаксимальныеПоказанияНачало.Район КАК Район,
	...
	|	МаксимальныеПоказанияНачало.ТарифнаяЗона КАК ТарифнаяЗона,
	|	СУММА(ВЫБОР
	|			КОГДА МаксимальныеПоказанияНачало.ПериодНачисленияКорректировки < МаксимальныеПоказанияПраво.ПериодНачисленияФактический
	|				ТОГДА -1
	...
	|	КОНЕЦ КАК ПоказанияТекущегоПериода
	|ИЗ
	|	МаксимальныеПоказанияНачало КАК МаксимальныеПоказанияНачало
	|		ЛЕВОЕ СОЕДИНЕНИЕ МаксимальныеПоказанияНачало КАК МаксимальныеПоказанияПраво
	...
	|			И МаксимальныеПоказанияНачало.ПериодНачисленияФактический = МаксимальныеПоказанияПраво.ПериодНачисленияФактический
	|			И (ВЫБОР
	...
	|	МаксимальныеПоказанияНачало.ПериодНачисленияФактический,
	|	МаксимальныеПоказанияНачало.ПериодНачисленияКорректировки,
	...
	Запрос.УстановитьПараметр("Организация", 						Организация);
	Запрос.УстановитьПараметр("Район", 								Район);
	Запрос.УстановитьПараметр("НачПокНеЯвляютсяКонПриУстПрибора",	энргРаботаСКонстантами.ЗначениеКонстантыНачальныеПоказанияПУНеРавныКонечнымВПериодеВведенияВЭксплуатациюПУ());
...
Процедура ДобавитьНаборВСхему(Схема,ИмяНабора,ТаблицаДляПомещения)
	...
	СоотвествиеПолей						= Новый Структура("Организация,Район,ТипНачисления","Организация","Район","ТипНачисления");
	...
	ИначеЕсли ИмяНабора  = "МаксимальныеПоказанияСрезПредыдущийПериод" Тогда
		...
		врПараметры  							= Новый Структура("ПараметрыВТ", врПараметрыВТ);
		РегистрыСведений.энргПредоставленныеПоказания.МаксимальныеПоказанияСрезСУчетомКорректировки(Схема,врПараметры,"МаксимальныеПоказанияСрезНаНачалоПерерасчета");
	...
	ИначеЕсли ИмяНабора = "ДниИспользованияУслугПриОтсутствии" Тогда
		...
		РегистрыНакопления.энргКоличествоДнейИспользованияУслугПриОтсутствии.КоличествоДнейИспользованияУслугПриОтсутствииКоллекцияПериодов(Схема, врПараметры, ТаблицаДляПомещения);
	...
	ИначеЕсли ИмяНабора = "ДниИспользованияУслугПриВременномОтключенииУслуги" Тогда
		...
		РегистрыНакопления.энргКоличествоДнейИспользованияУслугиПриОтключении.КоличествоДнейИспользованияУслугПриВременномОтключенииКоллекцияПериодов(Схема, врПараметры, ТаблицаДляПомещения);
	...
	ИначеЕсли ИмяНабора = "ДанныеСтабильныхПериодов" Тогда
		...
		Пакет.Операторы[0].ВыбираемыеПоля.Добавить("ЕСТЬNULL(энргПомещения.Наименование,"""")");
		Колонка 								= Пакет.Колонки[Пакет.Колонки.Количество()-1];
		Колонка.Псевдоним 						= "ПомещениеПредставление";

		Пакет.Операторы[0].Источники.Добавить("Справочник.энргСтроения","энргСтроения");
		УсловияСоединения 						= Новый Массив;
		УсловияСоединения.Добавить("НаборПериоды.Строение = энргСтроения.Ссылка");
		Пакет.Операторы[0].Источники.НайтиПоПсевдониму("энргАбоненты").Соединения.Очистить();
		Если Пакет.Операторы[0].Источники[0].Соединения.Добавить("энргСтроения",СтрСоединить(УсловияСоединения,Символы.ПС + " И ")) Тогда			 
			Соединение							= Пакет.Операторы[0].Источники[0].Соединения[Пакет.Операторы[0].Источники[0].Соединения.Количество() -1];
			Соединение.ТипСоединения 			= ТипСоединенияСхемыЗапроса.ЛевоеВнешнее;
		КонецЕсли;
		
		Пакет.Операторы[0].ВыбираемыеПоля.Добавить("ЕСТЬNULL(энргСтроения.Наименование,"""")");
		Колонка 								= Пакет.Колонки[Пакет.Колонки.Количество()-1];
		Колонка.Псевдоним 						= "СтроениеПредставление";
		...
		Пакет.Операторы[0].ВыбираемыеПоля.Добавить("ТИПЗНАЧЕНИЯ(НаборПериоды.ДокРегистратор)");
		Колонка 								= Пакет.Колонки[Пакет.Колонки.Количество()-1];
		Колонка.Псевдоним 						= "ТипЗначенияДокРегистратора";
		
		Пакет.Операторы[0].ВыбираемыеПоля.Добавить(
		"	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(НаборПериоды.ДокРегистратор) = ТИП(Документ.энргОперацияСУслугами)
		|			ТОГДА ВЫРАЗИТЬ(НаборПериоды.ДокРегистратор КАК Документ.энргОперацияСУслугами).ВидОперации
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ");
		Колонка 								= Пакет.Колонки[Пакет.Колонки.Количество()-1];
		Колонка.Псевдоним 						= "ВидОперации";
	...
	ИначеЕсли ИмяНабора  = "ЗначенияДополнительныхСоциальныхНормативов" Тогда
						
		РегистрыСведений.энргЗначенияДополнительныхСоциальныхНормативов.ЗначенияДополнительныхСоциальныхНормПотребленияКоллекцияПериодов(Схема,Параметры,ИмяТаблицыПериодов,ТаблицаДляПомещения);
	...
	ИначеЕсли ИмяНабора = "НастройкиРасчетовПриРегистрацииАктаНедопуска" Тогда
		РегистрыСведений.энргНастройкиРасчетовПриРегистрацииАктаНедопуска.НастройкиРасчетовПриРегистрацииАктаНедопускаКоллекцияПериодов(Схема, Параметры, ИмяТаблицыПериодов, ТаблицаДляПомещения);
		Пакет 									= Схема.ПакетЗапросов[Схема.ПакетЗапросов.Количество()-1];
		Оператор 								= Пакет.Операторы[0];
	...
	ИначеЕсли ИмяНабора = "НастройкиСниженияСтоимостиУслуг" Тогда
		РегистрыСведений.энргНастройкиСниженияСтоимостиУслуг.НастройкиСниженияСтоимостиУслугКоллекцияПериодов(Схема, Параметры, ИмяТаблицыПериодов, ТаблицаДляПомещения);
		
	ИначеЕсли ИмяНабора = "УвеличениеСниженияСтоимостиУслуг" Тогда
		РегистрыСведений.энргУвеличениеСниженияСтоимостиУслуг.УвеличениеСниженияСтоимостиУслугКоллекцияПериодов(Схема, Параметры, ИмяТаблицыПериодов, ТаблицаДляПомещения);
		
	ИначеЕсли ИмяНабора = "СостояниеВыполненияОбязанностейГарантирующегоПоставщика" Тогда
		РегистрыСведений.энргСостояниеВыполненияОбязанностейГарантирующегоПоставщика.ТочкиСНеВыполненнымиОбязанностямиГарантирующегоПоставщикаКоллекцияПериодов(Схема, Параметры, ИмяТаблицыПериодов, ТаблицаДляПомещения);
		
	ИначеЕсли ИмяНабора  = "НазначенияПриборовУчета" Тогда
		
		РегистрыСведений.энргНазначенияПриборовУчета.НазначенияПУКоллекцияПериодов(Схема,Параметры,ИмяТаблицыПериодов,ТаблицаДляПомещения);
