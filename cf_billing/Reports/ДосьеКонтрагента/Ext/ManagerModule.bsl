...
Процедура СформироватьРезультатГлавное(ИнформацияОСвязях, ИНН, ДанныеОтчета, ДанныеДосье, СтруктураГодов)
	...
	// Реквизиты
	ОбластиМакета.НаименованиеИАдрес.Параметры.НаименованиеСокращенное = ИнформацияОСвязях.НаименованиеСокращенное;
	ОбластиМакета.НаименованиеИАдрес.Параметры.ИНН = ИнформацияОСвязях.ИНН;
	ОбластиМакета.НаименованиеИАдрес.Параметры.КПП = ИнформацияОСвязях.КПП;
	ОбластиМакета.НаименованиеИАдрес.Параметры.ДатаРегистрации = Формат(ИнформацияОСвязях.ДатаРегистрации, "ДЛФ=D");
	Если ИнформацияОСвязях.Статус.Код <> Неопределено Тогда
		Если ИнформацияОСвязях.Статус.Код = "001" Тогда
			ОбластиМакета.НаименованиеИАдрес.Параметры.ДействующаяОрганизация = НСтр("ru='Действующая организация (по данным ЕГРЮЛ)'");
			ОбластиМакета.НаименованиеИАдрес.Область(3, 3, 3, 3).ЦветТекста = ЦветаСтиля.ДосьеХорошаяОценкаЦвет;
		Иначе
			ОбластиМакета.НаименованиеИАдрес.Параметры.ДействующаяОрганизация = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1 (по данным ЕГРЮЛ)'"),
				ИнформацияОСвязях.Статус.Наименование);
			ОбластиМакета.НаименованиеИАдрес.Область(3, 3, 3, 3).ЦветТекста = ЦветаСтиля.ДосьеПлохаяОценкаЦвет;
		КонецЕсли;
	КонецЕсли;
	
	// Адрес
	ОбластиМакета.НаименованиеИАдрес.Параметры.ЮридическийАдрес = ИнформацияОСвязях.ЮридическийАдрес.Представление;
	...
	Если КоличествоОрганизаций > 0 Тогда
		...
		ОбластиМакета.НаименованиеИАдрес.Параметры.ОрганизацииПоЭтомуАдресу = ОрганизацииПоЭтомуАдресу;
	КонецЕсли;
	ТабличныйДокумент.Вывести(ОбластиМакета.НаименованиеИАдрес);
	
	// Руководитель
	Если ИнформацияОСвязях.Руководители.Количество() = 0 Тогда
		ОбластиМакета.РуководительШапка.Параметры.Руководитель = "";
		ТабличныйДокумент.Вывести(ОбластиМакета.РуководительШапка);
	Иначе
		
		ВГраница = ИнформацияОСвязях.Руководители.Количество() - 1;
		Для Итератор = 0 По ВГраница Цикл
			ТекущийРуководитель = ИнформацияОСвязях.Руководители[Итератор];
			...
			Если КоличествоОрганизацийГдеУчредитель > 0 Тогда
				ВыводитьЕще = (ЗначениеЗаполнено(ТекущийРуководитель.ИНН)
					И ИнформацияОСвязях.Учредители.Найти(ТекущийРуководитель.ИНН, "ИНН") <> Неопределено)
					ИЛИ (НЕ ЗначениеЗаполнено(ТекущийРуководитель.ИНН)
					И ИнформацияОСвязях.Учредители.Найти(ТекущийРуководитель.Представление, "Учредитель") <> Неопределено);
...
Процедура ВывестиПоказателиВГлавное(ТабличныйДокумент, ИнформацияОСвязях, СтруктураГодов, ОбластиМакета)
	...
	Если ОткрытыеДанныеЗаГод <> Неопределено Тогда
		...
		ВыведенаСтрока = Ложь;
		ОткрытыеДанныеЗаПредыдущийГод = ИнформацияОСвязях.ОткрытыеГосударственныеДанныеФНС.Найти(ГодОткрытыеДанные - 1, "Год");
		...
		ВывестиСтрокуПоказателя(
			...
			Новый Структура("НазваниеПоказателя, ЗначениеПоказателя, ПредыдущееЗначениеПоказателя",
				...
				?(ОткрытыеДанныеЗаГод = Неопределено, Неопределено, ОткрытыеДанныеЗаГод.КоличествоСотрудников),
				?(ОткрытыеДанныеЗаПредыдущийГод = Неопределено, Неопределено, ОткрытыеДанныеЗаПредыдущийГод.КоличествоСотрудников)));
...
Процедура ВывестиРазделРуководители(ТабличныйДокумент, ИнформацияОСвязях, ПараметрыВывода)
	...
	Если ЭтоЮридическоеЛицо Тогда
		ДанныеРуководителей = ИнформацияОСвязях.Руководители;
	Иначе
...
Процедура ЗаполнитьСреднююРентабельностьОКВЭД2(Результат, ВидыДеятельности, Год, ИмяОбласти)
	...
	КодОКВЭД2 = ВидыДеятельности[0].Код;
	...
	ВозможныеКодыОКВЭД = ИерархияКодовОКВЭД(КодОКВЭД2);
	Для каждого ВозможныйКодОКВЭД Из ВозможныеКодыОКВЭД Цикл
		ОбластьСтроки = Макет.НайтиТекст(ВозможныйКодОКВЭД, , ОбластьКодОКВЭД2, Ложь, Истина);
